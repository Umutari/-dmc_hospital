...
if ($action === 'new' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$patientId || !is_array($items) || !count($items)) {
        ...
        $invId = execute(
            "INSERT INTO invoices (invoice_no,patient_id,total,paid,balance,status,notes,due_date,created_by) VALUES (?,?,?,0,?,?,?,?,?)",
            [$no, $patientId, $total, $total, 'issued', $notes, $dueDate ?: null, $uid]
        );
...
/* ── single invoice view ── */
if ($id) {
    $inv     = row("SELECT i.*, CONCAT(p.first_name,' ',p.last_name) AS pname, p.patient_no, p.phone
...
/* ── list: invoices created by this doctor ── */
$filter = $_GET['status'] ?? 'all';
$list   = [];
if (!$id && $action !== 'new') {
    $q = "SELECT i.*, CONCAT(p.first_name,' ',p.last_name) AS pname
          FROM invoices i JOIN patients p ON i.patient_id=p.id
          WHERE i.created_by=?";
    $p = [$uid];
    if ($filter !== 'all') { $q .= " AND i.status=?"; $p[] = $filter; }
...
<div class="page-header">
  <div><div class="page-title">Patient Invoices</div><div class="page-sub">Create and view invoices for your patients</div></div>
  <div class="d-flex gap-2">
    <?php if ($id || $action === 'new'): ?>
    <a href="/dmc/doctor/invoices.php" class="btn-dmc-outline"><i class="bi bi-arrow-left"></i> Back</a>
    <?php endif; ?>
    <a href="/dmc/doctor/invoices.php?action=new" class="btn-dmc"><i class="bi bi-plus-circle"></i> New Invoice</a>
  </div>
</div>
      ...
      <form method="POST">
        ...
        <div class="row g-3">
          ...
          <div class="col-12 d-flex gap-2 justify-content-end">
            <a href="/dmc/doctor/invoices.php" class="btn btn-secondary">Cancel</a>
...
<div class="dmc-card">
  <div class="d-flex justify-content-between align-items-center mb-3">
    <div class="dmc-card-title mb-0">My Invoices</div>
  ...
  <div class="table-responsive">
    <table class="table dmc-table mb-0">
      ...
      <tr>
        ...
        <td><a href="?id=<?= $inv['id'] ?>" class="btn btn-sm btn-outline-primary" style="font-size:10px">View</a></td>
      </tr>
      <?php endforeach; ?>
      <?php if (!$list): ?><tr><td colspan="8" class="text-center text-muted py-4">No invoices yet</td></tr><?php endif; ?>
      </tbody>
    </table>
  </div>
</div>
